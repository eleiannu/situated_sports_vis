...
public class UIUtilities {
  ...
  public static func createPoseOverlayView(
    ...
    public static func addPose(
      forPose pose: Pose, lineWidth: CGFloat, dotRadius: CGFloat,
      positionTransformationClosure: (VisionPoint) -> CGPoint, overlayView: UIView
    ) {
      let lowerBodyHeight: CGFloat =
        UIUtilities.distance(
          fromPoint: pose.landmark(ofType: PoseLandmarkType.leftAnkle).position,
    ...
    public static func createPoseOverlayViewOnBody(
      ...
      for (startLandmarkType, endLandmarkTypesArray) in UIUtilities.poseConnections() {
        ...
        for endLandmarkType in endLandmarkTypesArray {
          let endLandmark = pose.landmark(ofType: endLandmarkType)
          let startLandmarkPoint = minus(lhs: positionTransformationClosure(startLandmark.position), rhs: ankleDifference)
          let endLandmarkPoint = minus(lhs: positionTransformationClosure(endLandmark.position), rhs: ankleDifference)
      ...
      for landmark in pose.landmarks {
          let landmarkPoint = minus(lhs: positionTransformationClosure(landmark.position), rhs: ankleDifference)
        UIUtilities.addCircle(
          atPoint: landmarkPoint,
          to: overlayView,
          color: UIColor.blue,
